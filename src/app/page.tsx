...
export default async function Home(){
  ...
  return <>
    <section className="hero">
      <div className="hero-card"><div className="eyebrow">EC Creative Studios</div><h1>Campaign mail without the bloat.</h1><p>Import the Pixieset list, build a branded email, test it, send once, and permanently honor every opt-out.</p><div className="toolbar"><Link className="button" href="/campaigns">Build campaign</Link><Link className="button secondary" href="/contacts">Manage contacts</Link></div></div>
      <aside className="setup"><div className="eyebrow" style={{color:'#dce8df'}}>System status</div><h2>{supabaseReady&&resendReady&&addressReady&&adminReady?'Ready to send':'Setup still required'}</h2><p>Supabase: <strong>{supabaseReady?'configured':'blocked'}</strong></p><p>Resend: <strong>{resendReady?'configured':'blocked'}</strong></p><p>Postal address: <strong>{addressReady?'configured':'required'}</strong></p><p>Admin lock: <strong>{adminReady?'configured':'required'}</strong></p></aside>
    </section>
    ...
    <section className="panel"><h2>Safety rules</h2><p>CSV re-imports never reactivate an unsubscribed, bounced, complained, or manually suppressed contact. Campaign sends require a successful test first, a typed <strong>SEND</strong> confirmation, and a campaign lock so a double click cannot create a second blast.</p></section>
  </>
}
