...
export default async function CampaignsPage(){
  ...
  return <><section className="panel"><div className="eyebrow">Compose</div><h1>New campaign</h1><CampaignForm /></section><section className="panel"><h2>Campaigns</h2><div className="table-wrap"><table className="data"><thead><tr><th>Campaign</th><th>Subject</th><th>Status</th><th>Delivery</th><th>Actions</th></tr></thead><tbody>{campaigns?.length?campaigns.map(c=>{const m=c.resend_broadcast_id?(metricsByBroadcast.get(c.resend_broadcast_id)??blankMetrics()):blankMetrics();return <tr key={c.id}><td><strong>{c.name}</strong><br/><small>{new Date(c.created_at).toLocaleString('en-US',{timeZone:'America/New_York'})}{c.scheduled_at?<><br/>Schedule: {new Date(c.scheduled_at).toLocaleString('en-US',{timeZone:'America/New_York'})}</>:null}</small></td><td>{c.subject}</td><td><span className={`badge ${c.status==='sent'?'good':c.status==='failed'?'warn':''}`}>{c.status}</span></td><td><small>{m.delivered} delivered · {m.opened} opened · {m.clicked} clicked<br/>{m.bounced} bounced · {m.complained} complaints</small></td><td><div className="toolbar"><Link className="button ghost" href={`/campaigns/${c.id}`}>Preview / Edit</Link><CampaignActions campaignId={c.id} status={c.status} scheduledAt={c.scheduled_at}/></div></td></tr>}):<tr><td colSpan={5} className="empty">No campaigns yet.</td></tr>}</tbody></table></div></section></>
}
