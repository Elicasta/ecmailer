...
export function CampaignForm(){
  ...
  async function submit(e:React.FormEvent<HTMLFormElement>){e.preventDefault();const formElement=e.currentTarget;setBusy(true);setMessage('Saving…');const form=new FormData(formElement);const payload=Object.fromEntries(form.entries()) as Record<string,string>;if(payload.scheduledAt){const scheduled=new Date(payload.scheduledAt);if(Number.isNaN(scheduled.getTime())){setMessage('Choose a valid schedule time.');setBusy(false);return}payload.scheduledAt=scheduled.toISOString()}try{const res=await fetch('/api/campaigns',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const data=await res.json();if(!res.ok)throw new Error(data.error||'Could not save campaign');setMessage('Campaign saved.');formElement.reset();setTimeout(()=>location.reload(),500)}catch(err){setMessage(err instanceof Error?err.message:'Could not save campaign')}finally{setBusy(false)}}
