...
export async function POST(request:Request){
  try{
    ...
    const contacts=[...seen.entries()].map(([email,row])=>{const address=[row['Address Line 1'],row['Address Line 2']].filter(Boolean).join(', ')||row.Address||row.address||null;return {email,first_name:row['First Name']||row.first_name||null,last_name:row['Last Name']||row.last_name||null,company:row.Company||row.company||null,phone:row.Phone||row.phone||null,address,city:row.City||row.city||null,state:row['State/Province']||row.State||row.state||null,zip:row['Zip/Postal Code']||row.Zip||row.ZIP||row.zip||null,country:row.Country||row.country||null,notes:row.Notes||row.notes||null,source:'pixieset',source_type:(row.Type||row.type||'other').toLowerCase(),updated_at:new Date().toISOString()}})
