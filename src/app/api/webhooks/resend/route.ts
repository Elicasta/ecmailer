...
export async function POST(request:Request){
  try{
    ...
    const db=getSupabaseAdmin();const emailId=event?.data?.email_id||event?.data?.id||null;const broadcastId=event?.data?.broadcast_id||null;const recipient=Array.isArray(event?.data?.to)?event.data.to[0]:event?.data?.email||event?.data?.to||null;const type=String(event?.type||'unknown')
    const {error:insertError}=await db.from('ecm_email_events').upsert({webhook_id:webhookId||null,resend_email_id:emailId,broadcast_id:broadcastId,event_type:type,contact_email:recipient,payload:event},{onConflict:'webhook_id',ignoreDuplicates:true});if(insertError)throw insertError
    ...
    if(broadcastId&&recipient){
      const [{data:campaign},{data:contact}]=await Promise.all([db.from('ecm_campaigns').select('id').eq('resend_broadcast_id',broadcastId).maybeSingle(),db.from('ecm_contacts').select('id').eq('email',String(recipient).toLowerCase()).maybeSingle()])
