...
export async function POST(request:Request){
  ...
  try{
    ...
    const result=await resend.broadcasts.create({segmentId,from:getSender(),replyTo:process.env.RESEND_REPLY_TO?[process.env.RESEND_REPLY_TO]:undefined,name:campaign.name,subject:campaign.subject,previewText:campaign.preview_text||undefined,html,text,send:true})
    if(result.error)throw new Error(result.error.message)
    providerAccepted=true
    const broadcastId=result.data?.id??null
    const {error:updateError}=await db.from('ecm_campaigns').update({status:'sent',resend_segment_id:segmentId,resend_broadcast_id:broadcastId,sent_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq('id',campaignId).eq('send_token',sendToken);if(updateError)throw updateError
    return NextResponse.json({ok:true,targeted:contacts.length,broadcastId})
